...
The accompanying repository archive is tagged \texttt{v1.0.5}.

\section{Software Environment}

The distributed scripts use Python 3 and standard numerical packages.  The core
dependencies are \texttt{numpy} for finite-array enumeration, Monte Carlo CPA
simulation, and linear algebra, \texttt{scipy} for symmetric eigenspace
computations, and \texttt{matplotlib} for figure generation.  The deterministic
tables are obtained by exhaustive enumeration.  The CPA validation uses a
fixed pseudorandom seed, \texttt{20260622}, Gaussian noise standard deviation
\(\sigma=2\), and \(3000\) trials per S-box.  When an additional 4-bit
bijection is used in \texttt{verify\_curvature.py}, the script uses a fixed
table explicitly listed in the source code.
...
figures.  The second command regenerates the CPA Monte Carlo figure and the
CSV summary of success probabilities and key ranks.  The third command
constructs the boundary covariance matrix,
projects onto the active eigenspace, computes the projected third cumulant
contractions, and evaluates the curvature diagnostic.  The fourth command is a
...
\begin{enumerate}[label=\normalfont(\roman*)]
  ...
  \item Monte Carlo CPA success probabilities and true-key ranks;
...
\texttt{simulate\_cpa.py} & Regenerates CPA simulation figure and CSV results. \\
\texttt{cpa\_simulation\_results.csv} & Records simulated success probabilities and ranks. \\
...
\texttt{supplementary\_README.md} & Lists commands and expected outputs. \\
