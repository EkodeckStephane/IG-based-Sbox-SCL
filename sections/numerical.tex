...
Table~\ref{tab:gap-distribution} is the part of the experiment that is most
directly relevant to CPA.  PRESENT and SKINNY have a smaller hardest-key gap
than GIFT under this leakage model, so the model-based estimate
\eqref{eq:gap-sample-estimate} predicts a larger number of traces to obtain
the same expected odds margin against the hardest wrong key.  This observation
does not imply a full-cipher security ordering, but it shows why the proposed
separation between scalar invariants and pairwise discrimination structure is
necessary.
